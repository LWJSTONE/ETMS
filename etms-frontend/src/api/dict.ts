...
// 获取字典类型列表
export function getDictTypeList(params: PageParams & { dictName?: string; dictType?: string; status?: number }): Promise<ApiResponse<PageResult<DictType>>> {
  return request.get('/system/dict/types', params)
}

// 获取字典类型详情
export function getDictTypeDetail(id: number): Promise<ApiResponse<DictType>> {
...
// 获取字典数据列表
export function getDictDataList(dictTypeId: number): Promise<ApiResponse<DictData[]>> {
